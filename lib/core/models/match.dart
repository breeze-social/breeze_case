...
class Match implements Comparable<Match> {
  ...
  MatchedUser get thisUser => matchedUsers.firstWhere((mu) => mu.userId == locator<UserRepository>().userId);

  /// Returns the other user in the match.
  MatchedUser get otherUser => matchedUsers.firstWhere((mu) => mu.userId != locator<UserRepository>().userId);
  ...
  int compareTo(Match other) {
    if (plannedOn != null && other.plannedOn != null) {
      return plannedOn!.compareTo(other.plannedOn!); // natural order (first datetime comes first)
    }
    return other.createdAt.compareTo(createdAt); // reversed natural order (last datetime comes first)
  }
